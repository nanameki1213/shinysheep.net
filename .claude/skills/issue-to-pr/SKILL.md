---
name: issue-to-pr
description: GitHub issue を起点に、ブランチ作成 → 実装 → コミット → push → PR 作成までの一連の流れを実行する。「issue #12 を対応して PR を出して」「このイシューを実装してプルリク作って」のように、issue 番号や issue 内容から作業を始めて PR にまとめたいときに使う。
---

# issue-to-pr

GitHub issue を起点に、ブランチを切って実装し、コミット・push して PR を作成するまでを一気通貫で行う。

このリポジトリの前提:
- リモート: `origin` = `https://github.com/nanameki1213/shagi.dev`
- メインブランチ: `main`
- frontend は **yarn**（`frontend/yarn.lock`）。lint/build もここで実行する。
- `gh` CLI が使える。issue 取得・PR 作成は `gh` を使う。
...
### 8. 仕上げ
作成した PR の URL と、行った変更・検証結果の要約を報告する。
